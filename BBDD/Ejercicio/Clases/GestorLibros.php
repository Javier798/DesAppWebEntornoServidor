 ...
 */
class GestorLibros implements AccionesBD
{
    ...
    {
        try {
            $sql = "INSERT INTO `libros`(`TITULO`, `NUMPAGINAS`, `VENDIBLE`, `FECHAPUBLI`) VALUES ('$array[TITULO]', $array[NUMPAGINAS], $array[VENDIBLE], '$array[FECHAPUBLI]')";
            $resultado = $this->conexion->query($sql);
            if ($resultado) {
                echo "Libro dado de alta correctamente. Hay ahora mismo " . $resultado->rowCount() . " libros. <br>";
            } else {
    ...
    function actualizar($array)
    {
        try {
            ...
            $sql = "UPDATE LIBROS SET $sentencia WHERE CODLIBRO =$codigo;";
            $resultado = $this->conexion->query($sql);
            if ($resultado) {
 ...
 */
    ...
    {
        try {
            $sql = "DELETE FROM LIBROS WHERE CODLIBRO = $id";
            $resultado = $this->conexion->query($sql);
            if ($resultado) {
    ...
    function obtenerHobbies()
    {
        try {
            ...
            foreach ($libros as $libro) {
                $contador++;
                $objeto = new Libro($libro["TITULO"], $libro["NUMPAGINAS"], $libro["VENDIBLE"], $libro["FECHAPUBLI"]);
                array_push($catalogoLibros, $objeto);
            }
    ...
    function obtenerHobbiesFiltros($array)
    {
        ...
        $sql = "SELECT * FROM libros WHERE $sentencia";
